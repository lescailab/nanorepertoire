...
input_ch = Channel
.fromFilePairs('/Users/bagordo/Desktop/all/all_bioinformatics/nf-core-nanorepertoire/data/*_{1,2}_dummy2.fastq')
.map{ name, reads ->
...
workflow {
    trimmed = CUTADAPT(input_ch)
    mixed =   FLASH(CUTADAPT.out.reads)
    renamed = RENAME(FLASH.out.merged, single_end = true)
}
